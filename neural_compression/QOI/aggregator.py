...
class QoIAggregator(nn.Module):
    """
    Aggregates multiple QoIs into a single scalar loss.
    Optionally uses uncertainty weighting (Kendall & Gal style):
        sum_i [ exp(-2 s_i) * L_i + 2 s_i ], where s_i = log_sigma_i
    """
    def __init__(self, qois, use_uncertainty: bool = True, init_log_sigma: float = -2.0):
        super().__init__()
        self.qois = nn.ModuleList(qois)
        self.use_uncertainty = use_uncertainty
        if use_uncertainty:
            self.log_sigmas = nn.Parameter(torch.full((len(qois),), init_log_sigma))
        else:
            self.register_parameter("log_sigmas", None)

    def forward(self, x_enh, x_true, x_prime, *,
                field_names=None, aux=None):
        """
        x_enh, x_true, x_prime: [N, C, D, H, W] (or [N, C, H, W] for 2D)
        field_names: list[str] of length C (optional, used by QoIs)
        aux: dict for QoI-specific extras, e.g., spacing, channel indices, masks
        """
        losses = []
        all_stats = {}
        for i, q in enumerate(self.qois):
            li, stats_i = q(x_enh, x_true, x_prime, field_names=field_names, aux=aux)
            losses.append(li)
            # prefix stats keys with qoi name
            for k, v in stats_i.items():
                all_stats[f"{q.name}/{k}"] = v

        if not losses:
            return x_enh.new_zeros(()), all_stats

        L = torch.stack(losses)  # [num_qois]
        if self.use_uncertainty:
            # sum_i exp(-2 s_i) * L_i + 2 s_i
            weight = torch.exp(-2.0 * self.log_sigmas)
            loss = (weight * L).sum() + 2.0 * self.log_sigmas.sum()
            # expose weights for logging
            for i, q in enumerate(self.qois):
                all_stats[f"{q.name}/weight"] = weight[i].detach()
                all_stats[f"{q.name}/log_sigma"] = self.log_sigmas[i].detach()
        else:
            loss = L.mean()
